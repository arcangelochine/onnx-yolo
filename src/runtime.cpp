...
void Runtime::config_session() {
...
#ifdef _WIN32
  cuda_available =
      nullptr == OrtSessionOptionsAppendExecutionProvider_CUDA(options, 0);
#elif __linux__
  cuda_available =
      nullptr == OrtSessionOptionsAppendExecutionProvider_CUDA(options, 0);
#elif __APPLE__
  std::cout << "CUDA is not supported on macOS." << std::endl;
#endif
  ...
  if (!cuda_available) {
    ...
    if (core_ml_available) {
      std::cout << "Falling back to CoreML Execution Provider" << std::endl;
    } else {
